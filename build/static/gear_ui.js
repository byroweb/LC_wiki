/* Equipment builder.
 *
 * The maths here is the maths the server runs, and nothing else:
 *
 ...
 *
 * A swing lands when randominc(attack roll) > randominc(defence roll).  Both are
 * uniform over 0..n, so that comes out as the closed form in hitChance() below,
 * and the damage of a landed hit, randominc(max hit), averages max hit / 2.
 *
 * Everything the page needs about items, monsters and dragonfire is in
 * data/gear.js, generated by build/gear.py straight from the content.
 */
'use strict';
...
// P(randominc(a) > randominc(d)) for uniform rolls over 0..a and 0..d
...
function attackRate() {
  ...
  var rows = styleRows(), row = rows[Math.min(S.style, rows.length - 1)];
  if (row && row[1] === STYLE_RANGED_RAPID) rate -= 1;     // [label,player_ranged_attack]
...
/* Everything [proc,player_combat_stat] works out, for the current set and style. */
function playerStats() {
  var b = bonuses();
  var rows = styleRows();
  var row = rows[Math.min(S.style, rows.length - 1)] || ['Punch', 0, DT_CRUSH];
  var damagestyle = row[1], damagetype = row[2];
  var sb = G.styleBonus[damagestyle] || G.styleBonusDefault;

  var effAttack = effectiveStat(S.lv.attack, prayerMultiplier('attack'));
  var effStrength = effectiveStat(S.lv.strength, prayerMultiplier('strength'));
  var effDefence = effectiveStat(S.lv.defence, prayerMultiplier('defence'));
  var effMagic = effectiveStat(S.lv.magic, 100);
  // magic defence is blended before the style bonuses go on, as in the script
  var effMagicDefence = Math.trunc((7 * effMagic + 3 * effDefence) / 10) + 8;
  var effRanged = effectiveStat(S.lv.ranged, 100);
  ...
  // indexed by damage type (stab, slash, crush, ranged, magic), which is not the
  // order ~equip_get_bonuses hands the bonuses back in
  var atk = [combatStat(effAttack, b[0]), combatStat(effAttack, b[1]), combatStat(effAttack, b[2]),
             combatStat(effRanged, b[4]), combatStat(effMagic, b[3])];
  var def = [combatStat(effDefence, b[5]), combatStat(effDefence, b[6]), combatStat(effDefence, b[7]),
             combatStat(effDefence, b[9]), combatStat(effMagicDefence, b[8])];
  var maxhit = damagetype === DT_RANGED ? combatMaxhit(combatStat(effRanged, b[12]))
                                        : combatMaxhit(combatStat(effStrength, b[10]));
  return {
    bonuses: b, style: row, damagestyle: damagestyle, damagetype: damagetype,
    attackRolls: atk, defenceRolls: def, maxhit: maxhit, rate: attackRate(),
    // the roll this style actually attacks with
    attackRoll: atk[Math.min(damagetype, 4)],
    ranged: damagetype === DT_RANGED
  };
}

function playerDefenceRoll(ps, damagetype) {
  // [proc,player_defence_roll_specific]; ^melee_style is not one of its cases,
  // so a monster that carries it is read as crush, the damagetype default
...
// gear.js lists the npc defence bonuses in param order; this is which one each
// damage type rolls against
var NPC_DEF_BONUS = [5, 6, 7, 9, 8];

function npcDefenceRoll(mon, damagetype) {
  // [proc,npc_defence_roll_specific]
  var stat = damagetype === DT_MAGIC ? mon.st[3] : mon.st[2];
  var eff = effectiveStat(stat, 100) + 9;           // npcs are always on 'controlled'
  var bonus = mon.b[NPC_DEF_BONUS[Math.min(Math.max(damagetype, 0), 4)]];
  return combatStat(eff, bonus);
}
